...
At first, check if the `stash-recovered-adv` StatefulSet's pod has gone into `Running` state by the following command,
...
Verify that the sample data has been restored in `/source/data` directory of the `stash-recovered-adv` StatefulSet's pod using the following command,
...
kubectl delete -n demo statefulset stash-recovered-adv
